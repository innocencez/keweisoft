...
(function($) {
	...
	$.omWidget("om.omPanel" , {
		...
		_create: function(){
			if(this.element.parent().length > 0 && this.element.parent()[0].tagName == "BODY") {
				this.options.width = "99%";
			    this.element.addClass("om-panel-body om-widget-content").css({border:"none", padding: 0, overflow: "hidden"})
			    	.wrap("<div class='om-widget om-panel'></div>");
		    	this.element.parent().css({border: "1px solid #c8c8c8",margin: "0 auto",width: "99%"});
			} else {
...
;(function($) {
    ...
    $.omWidget('om.omButton', {
        ...
        _create : function() {
            ...
            var wrapperSpan = $( '<span>' ).addClass( 'dialogbtns om-btn om-state-default'),
            ...
            this.element.addClass( 'om-btn-txt' )
                .css( {'background-position':'center center','background-repeat':'no-repeat'} )
                .wrap( wrapperSpan )
                .before( leftSpan )
                .after( rightSpan )
                .wrap( centerSpan );
        ...
        changeIcons : function( icons ){
        	if( !this.options.disabled ){
	            if( icons ){
	            	...
	            	icons.left?this.element.prev().attr( 'src',icons.left ):null;
	            	icons.right?this.element.next().attr( 'src',icons.right ):null;
	            }
            }
        },
        
        changeClick : function(onClick) {
        ...
        _initButton : function(){
        	...
        	if( options.icons.left ){
        	    ...
        	    if( label != '' ){
        	        $( '<img>' ).attr( 'src',options.icons.left ).css( {'vertical-align':'baseline','padding-left':'3px'} ).insertBefore( element );
        	    }else{
        	        $( '<img>' ).attr( 'src',options.icons.left ).css( 'vertical-align','baseline' ).insertBefore( element );
        	    }
        	}
...
;(function($) {
	...
	$.omWidget('om.omCombo', {
        ...
        _init:function(){
            ...
            if (options.float) {
            	inputEl.parent().css("float", options.float);
            }
            if (options.height) {
            	inputEl.parent().css("height", options.height);
            	inputEl.css("height", options.height-2);
            	this.expandTrigger.css("height", options.height);
            }
        ...
        _create:function(){
            ...
            valueEl.hide();
            this.dropList = $($('<div class="om-widget"><div class="om-widget-content om-droplist"></div></div>').css({position:'absolute', zIndex:this.options.zIndex}).appendTo(document.body).children()[0]).hide();
        },
        /**
        ...
        getSelectData:function () {
        	//如果已经存在dataSource则直接取出
            var sources = this.options.dataSource;
            var checkSource = [];//选中行数组
            var checkVal=this.value();//选中值
            for (var i=0;i<sources.length;i++) {
            	//判断是否有选中的值，有则往checkSource中放
            	if ((','+checkVal+',').indexOf(','+sources[i].value+',')>=0) {
            		checkSource[checkSource.length]=sources[i];
            	}
            }
            return checkSource;
        },
         ...
         */
         ...
         getValueText:function(){
           	...
           	for(var i = 0; i < data.length; i ++) {
  				if(("," + val + ",").indexOf("," + data[i].value + ",") >= 0) {
  					resultData[resultData.length] = data[i].text;
  				}
           	}
                ...
                if (!self.dataHasLoaded) {
                    if(!expandTrigger.hasClass('om-loading')){
                        ...
                        if (typeof(options.dataSource) == 'object') {
                        	if(options.eachAjaxLoad) {
            					var toHtData = self.getFilterData();
            					var tmpJsonData = "tableId="+options.tableId+"&pageId="+options.pageId+"&ifGetAll=1";
            					if(toHtData != null && toHtData != undefined) {
            						for(var o in toHtData){  
			         					tmpJsonData += "&" + o + "=" + toHtData[o];
			    					} 
			    				}
                        		self._ajaxLoad('model_ajaxList.do?'+tmpJsonData);
                        	} else {
        ...
        getFilterData : function(data) {
            ...
            if(toHtData != null && toHtData != undefined) {
			    ...
			    data.tmpJsonData = tmpJsonData;
            }
            //判断自身是否有needFilter元素 如果有则获取外层tr的id属性并传至后台
            var needFilter = el.attr("needFilter");
            if(needFilter != null && needFilter.length > 0) {
            	var needFilterArr = needFilter.split(",");
            	var tmpId = el.closest("tr").data("id");
           		$(needFilterArr).each(function(i, one){
           			if(data.tmpJsonData == null) {
           				data.tmpJsonData = "";
           			}
           			var tmpOneArr = one.split(":");
           			var plugName = tmpOneArr[0];
           			var tmpFieldId = tmpOneArr[1];
           			var tmpArr = $("input[name$='" + tmpFieldId + "']");
           			//首先判断同级的 tr下有没有 然后判断父级的table下有没有
           			//我把这里之前的所有的用prevName做条件的都改为了name,因为现在得判断缺少一本分判断，而且name的效果也是一样的（王启玉）
           			var prevTrEle = el.closest("tr").find("input[name$='" + tmpFieldId + "']");
           			var ifJx = true;
           			if(prevTrEle.length == 0) {
           				prevTrEle = el.closest("table").parent().closest("table").find("input[name$='" + tmpFieldId + "']");
           				//20160426 条件判断错误  添加了 prevTrEle.length >0 &&   （王启玉）
           				if(prevTrEle.length >0 && (prevTrEle.attr("id") + "_childTable") == el.closest("table").attr("id")) {
           					prevTrEle = [];
           					ifJx = false;
           				} else {
           					//20160426条件判断错误  添加了 if(prevTrEle.length == 0) { （王启玉）
           					if(prevTrEle.length == 0) {
           						prevTrEle = el.closest("table").find("input[name$='" + tmpFieldId + "']");
           					}
           				}
           			} 
           			if(ifJx) {
	           			if(prevTrEle.length == 0) {
	           				prevTrEle = $("input[hideFieldId='" + tmpFieldId + "']");
	           			}
	           			if(prevTrEle.length > 0) {
	           				if(prevTrEle.attr("type") == "hidden") {
	           					tmpId = prevTrEle.val();
	           				} else if (prevTrEle.parent().hasClass('om-combo')) {//判断是否为下拉框类型字段
	           					tmpId = prevTrEle.omCombo('value');
	           				} else {
		           				var inputData = prevTrEle.data("inputData");
		           				if(inputData != null) {
			           				tmpId = 0;
			           				$(inputData).each(function(j, oneData){
	           				...
	           				//依旧没有找到元素的话 获取页面是否是 修改页面或者联动添加页面 获取元素的id传至后台
	           				if($("input[fieldrole=0]").length > 0) {
		           				tmpId = $("input[fieldrole=0]").val();
	           				}
	           			}
	           		}
           			if(tmpId != undefined) {
           				data.tmpJsonData = data.tmpJsonData + "&" + tmpOneArr[0] + "=" + tmpId;
           			}
           		});
            }
            return data;
        ...
        _showDropList:function(){
            ...
            bodyheight = dropList.closest("body").height();
            if((bodyheight-inputPos.top-span.outerHeight()) > dropListContainer.outerHeight()){
            	...
            	dropListContainer.css({
	                'left': inputPos.left,
	                'top': inputPos.top - dropListContainer.outerHeight()
	            });
	            dropList.css({
	                'border-top':'1px solid #64a0fb'
	            });
            }
            dropList.show();
...
$.omWidget("om.omDialog", {
	...
	_create: function() {
		...
		var self = this;
		self.options.oldPar = self.element.parent(); 
	...
	closeAndRemove: function() {
		...
		self.element.html("");
		//克隆弹层div
		var cloneDiv=self.element.clone(false);
		//删除弹层div样式
		cloneDiv.removeAttr("class");
		cloneDiv.removeAttr("style");
		var tmpId = self.element.attr("id");
		var parentEle = self.element.parent(".om-dialog"); 
		var oldPar = this.options.oldPar;
		if(oldPar == null) {
			oldPar = $("body");
		}
		...
		parentEle.remove();
		/*if (self.originalTitle) {
			cloneDiv.attr("title",self.originalTitle);
			oldPar.append("<div id=\"" + tmpId + "\" style=\"\" title=\"" + self.originalTitle + "\" ></div>");
		} else {
			oldPar.append("<div id=\"" + tmpId + "\" style=\"\" ></div>");
		}*/
		oldPar.append(cloneDiv);
		return self;
	},
	...
	_createButtons: function(buttons) {
		var self = this,
			...
			uiButtonSet = $( "<div></div>" )
				.addClass( "om-dialog-buttonset" ).css({ 
		...
		if (hasButtons) {
			$.each(buttons, function(name, props) {
				...
				if ($.fn.omButton) {
					button.omButton({"onClick": props.click});
				}
			});
			uiDialogButtonPane.appendTo(self.uiDialog);
			uiButtonSet.children().css("margin","0 5px");
		}
	},
	...
	_makeDraggable: function() {
		...
		self.uiDialog.omDraggable({
			...
			onStop: function(ui, event) {
				...
				heightBeforeDrag = 'auto';//ztx设置窗口移动后高度为auto。
	...
	_makeResizable: function(handles) {
		...
		self.uiDialog.omResizable({
			...
			start: function(event, ui) {
				...
				$(this).children('.om-dialog-content.om-widget-content').css({'min-height':'42px','max-height':''}).parent().css('max-height','');//ztx调整弹出层的高度设置。
			},
			resize: function(event, ui) {
	...
	_size: function() {
		...
		mixContentHeight = Math.max( 0, options.maxHeight - nonContentHeight );
		...
		if ( options.height === "auto" ) {
			...
			if ( $.support.minHeight ) {
				...
				if(mixContentHeight > 0) {
					this.element.css({
						maxHeight: mixContentHeight
					});
				}
			} else {
				this.uiDialog.show();
		...
		$(this.uiDialog).children('.om-dialog-content.om-widget-content').css('height','auto');
	//	thisBox.children('.om-dialog-content.om-widget-content').children('div').addClass('background1');
	}
});
...
(function($){
    ...
    $.omWidget('om.omFileUpload', {
        ...
        _setOption : function(key, value) {
            ...
            var dynOpts = ['buttonImg','buttonText','fileDesc','fileExt','height','action','sizeLimit','width','queueSizeLimit'];
        ...
        _create : function() {
            ...
            element.parent().find("object").css({"float":"left","margin-right":"10px"});
        }
    });
...
;(function($) {
    $.omWidget('om.omGrid', {
        options:/** @lends omGrid#*/{
             ...
             * @default false
             * @example
             * $('.selector').omGrid({pluralSelectIgnorePage : true});
             */
            pluralSelectIgnorePage:false,
            
            /**
             * 初始选中值 复选并忽略 分页是 默认选中   pCode为选中用参数  pId当前选中数组
             * @type Json
             * @default {}
             * @example
             * $('.selector').omGrid({allSelectedIgnorePageIndex : []);
             */
            allSelectedIgnorePageIndex:[],
             ...
             * @example
             *  $(".selector").omGrid({
             *      afterRenderer:function(data,testStatus,XMLHttpRequest,event){
             *          alert('fetch data success,got '+data.rows+' rows');
             *      }
             *  });
             */
        ...
        _create:function(){
            var options=this.options,el=this.element.show() // show if hidden
                ...
                .append('<tbody></tbody>');
            var style = "";
            if(tySelect_xqy != null && tySelect_xqy == 0) {
            	tySelect_xqy = options.colModel;
            }
            //表格最后默认多加一个空列，
            options.colModel[options.colModel.length]={header : '', name : '', align : 'left'} ;
            if(this.element.parent()[0].tagName == "BODY") {
            	style="margin: 0 auto; width: 99%;"
            }
            el.wrap('<div class="om-grid om-widget om-widget-content" style="' + style + '" ><div class="bDiv" style="width:auto"></div></div>').closest('.om-grid');
            
            
            var parent = el.parents(":hidden");
        	var tmp = []
         ...
         //   this.loadMask=$('<div class="gBlock"><div align="center" class="gBlock-valignMiddle" ><div class="loadingImg" style="display:block"/></div></div>')//ztx添加新的页面加载提示动态效果*/
            this.loadMask=$('<div class="gBlock" style="display:block;"><div class="content_load_box"><div class="content_load"><div></div><div></div></div><em>努力加载中...</em></div></div>')
                    .mousedown(function(e){
        ...
        _init:function(){
            ...
            this._populate();
            this.options.tmpParHide.each(function(){
        		$(this).hide();
        	});
        	...
        	//判断是否有默认选中值 如果有则调用 默认选中
        	var key = $el.data("key");
        	var selectedIndex = $el.data("selectedIgnorePageIndex");
        	if(key != null && selectedIndex != null) {
	        	$el.omGrid("setAllSelectedIgnorePageIndex", selectedIndex);
	            $el.omGrid("setSelections", null);
	        }
        },
        
        _winInitResize : function() {
        	var windowHeight = $(document).height(true);
			var $table = this.element.closest("div.om-grid");
			if($table.length > 0 && $table.offset() != null && $table.offset().top > 0) {
				var outerHeight = $table.offset().top;
				var tmpHeight = windowHeight - outerHeight - this.options.buttomHeight;
				if(tmpHeight <= 100) {
					tmpHeight = 300;
				}
	       		this.resize({height:tmpHeight});
       		}
        },
        ...
        _resetHeight : function(){
            ...
            //注：为了解决部分页面高度获取问题特意处理的
            if($grid.children('.bDiv').outerHeight()<2){
            	$grid.children('.bDiv').css('height','auto');
            }
            $(".om-grid").closest("table[id^='listTableId']").find("div[id^='expand']").css("border","none");
        },
        ...
        _buildTableHead:function(){
            ...
            //渲染序号列            
            if(op.showIndex){
                var cell=$('<th></th>').attr({axis:'indexCol',align:'center'}).addClass('indexCol').append($('<div style="text-align:center;width:25px;"></div>'));
                if(op.setFieldFun){
                	cell =$('<div class="indexheader" style="text-align:center;width:25px;"></div>');
                	var userId = document.getElementById('operamaskJs');
        			if(userId!=null && userId.getAttribute('data') == 0) {//根据登录人过滤显示列设置按钮
                		cell.append('<span class="setTableLine" ondblclick="setPageField(this,' + op.tableId + ',' + op.pageId + ')" ></span>'); 
                	}
                	cell= $('<th></th>').attr({axis:'indexCol',align:'center'}).addClass('indexCol').append(cell);
                }
                tr.append(cell);
                indexWidth=25;
            }
            //渲染checkbox列
            if(!op.singleSelect){
            	var ckcontent = '<div class="checkboxheader" style="text-align:center;width:17px;"><span class="checkbox"/></div>';
                if(!op.checkAllDefault) {
	            	ckcontent = '<div class="checkboxheader" style="text-align:center;width:17px;"></div>';
	            }
                var cell=$('<th></th>').attr({axis:'checkboxCol',align:'center'}).addClass('checkboxCol').append($(ckcontent));
                tr.append(cell);
                checkboxWidth=17;
            }
            //渲染colModel各列
        ...
        _appendTableHead:function(newColModel){
            var op=this.options,
                el=this.element,
                grid = el.closest('.om-grid'),
                cms=this._getColModel(),
            ...
            for (var i=newColModel.length-3,len=newColModel.length;i<len;i++) {
                var cm=cms[i],cmWidth = cm.width || 60,cmAlign=cm.align || 'center';
                if(cmWidth == 'autoExpand'){
                    cmWidth = 0;
                    autoExpandColIndex = i;
        ...
        _appendColsResizable:function(){
            var self=this,
                ...
                var col = this._getColModel();
            $('th[axis^="col"]',self.thead).children("div").each(function(){
            	if(!($(this).hasClass("om-resizable"))) {
            		$(this).omResizable({
		                handles: 'e',//只可水平改变大小
		                containment: 'document',
		                minWidth: 60,
        ...
        _buildPagingToolBar:function(){
            ...
            var self=this,
                ...
                pDiv= this.pDiv;
            var userId = document.getElementById('operamaskJs');
            var apHtml='<div class="pDiv2" ';
        	    if(userId!=null && userId.getAttribute('data') == 0){//根据登录人过滤列表内容设置按钮
        	    	if(op.tableId != undefined && op.pageId != undefined){
	                    apHtml +=' name="tabContentSet" ondblclick="setTableInfo(this,' + op.tableId + ',' + op.pageId + ');" ';
        	    	}
                }
                apHtml +='>'+
                '<div class="pGroup">'+
                ...
                '</div>'+
                '<div class="pGroup">'+
                    '<div class="pNext pButton om-icon"><span class="om-icon-seek-next">下一页</span></div>'+
                    '<div class="pLast pButton om-icon"><span class="om-icon-seek-end">末页</span></div>'+
                '</div>'+
                '<div class="btnseparator"></div>'+
                '<div class="pGroup"><span class="pLimit">单页条数<input style="height:20px;border: 1px solid #bfbfbf;" type="text" size="4" value="'+this.options.limit+'"/></span></div>'+
                '<div class="pGroup"><span class="pControl"></span></div>'+
                '<div class="btnseparator"></div>'+
                '<div class="btnseparator"></div>'+
                '<div class="pGroup">'+
                    '<div class="pFirst pButton om-icon"><span class="om-icon-seek-start">首页</span></div>'+
                    '<div class="pPrev pButton om-icon"><span class="om-icon-seek-prev">上一页</span></div>'+
                '</div>'+
                '<div class="btnseparator"></div>'+
                '<div class="pGroup"><span class="pPageStat"></span></div>'+
            	'</div>';
            pDiv.show().html(apHtml);
            var pageText = $.om.lang._get(op,"omGrid","pageText").replace(/{totalPage}/, '<span>1</span>').replace(/{index}/, '<input style="height:20px;border: 1px solid #bfbfbf;" type="text" size="4" value="1" />');
            ...
            $('.pLimit input', pDiv).keydown(function(e) {
                if (e.keyCode == $.om.keyCode.ENTER) {
					self._changeLimit();
				}
            });
        ...
        _changeLimit : function() { // change page
            var el=this.element,
                op=this.options,
                grid = el.closest('.om-grid'),
                oldLimit = this.options.limit;
                newLimit = this.options.limit;
                
	            var lm = parseInt($('.pLimit input', el.closest('.om-grid')).val());
	            if (isNaN(lm)) {
	                lm = oldLimit;
	            }
	            if (lm < 1) {
            ...
            //触发事件
            if(this._trigger("onLimitChange",null,newLimit)===false){
                return;
            }
            //修改pageData
            this.options.limit=newLimit;
            //$(".bDiv").find("table").find("tr").remove();
            //刷新数据
            this._populate();
        },
        //刷新数据
        _populate : function() { // get latest data
			...
			if(this.options.height == "auto") {
				 param.limit = limit + "_flow";       
				 op.flowLimit = limit + "_flow"; 
            } else {
            	 param.limit = limit + "_flow";       
				 op.flowLimit = limit + "_flow"; 
            }
            if(op.flowLimit) {
            	param.limit = op.flowLimit;
            }
            //添加是否 ajax进行记录数的查询
            param.ifAjaxCount = op.ifAjaxCount;
            
            if(op.fixedData) {
            	var onSuccess = op.onSuccess;
                self._addData(op.fixedData);
                for(var i=0 , len=op._onRefreshCallbacks.length; i<len; i++){
                	op._onRefreshCallbacks[i].call(self);
                }
                self._trigger("onRefresh",null,nowPage,op.fixedData.rows);
                loadMask.hide();
                self.loading = false;
                if(op.pluralSelectIgnorePage || op.singleSelect) {
                	self.setSelections();
                }
                var afterRenderer = op.afterRenderer;
                if(self.options.countId.length > 0) {
                	$(self.options.countId).each(function(i, one){
                		$("#" + one).text(op.fixedData[one]);
                	});
                }
            } else {
	            $.ajax({
	                ...
	                success : function(data,textStatus,request) {
	                    ...
	                    if (typeof(afterRenderer) == 'function') {
	                        self._trigger("afterRenderer",null,data,textStatus,request);
	                    }
	                    //绑定 列上的单击和双击事件 
	                    self._appendTdClick();
	                    ...
	                    if($.isArray(self.reloadChecked)) {
	                    	self.setSelections(self.reloadChecked);
	                    	self.reloadChecked = null;
	                    }
	                    //是否ajax加载 记录数 是的话进行ajax请假 加载记录数
	                    if(op.ifAjaxCount) {
	                    	param.isOnlyCount = true;
	                    	op.isOnlyCount = true;
	                    	$.ajax({
				                type : op.method,
				                url : op.dataSource,
				                ...
				                limit : op.limit,
				                dataType : 'json',
				                success : function(countData,textStatus,request) {
				                	data.total = countData.total;
				                	data.havingData = countData.havingData;
	                    			//给表格头 追加统计值
	                    			self._processHavingData(data); 
	                    			self._addData(data);
	                    		},
        ...
        //处理聚合汇总的信息
        _processHavingData:function(data){
        	var self=this;
        	var havingData = data.havingData;
                    if(havingData != null && havingData != undefined) {
                    	for(var key in havingData){  
                    		var value = havingData[key];
                    		var name = key.substring(0,key.lastIndexOf("_"));
                    		var type = key.substring(key.lastIndexOf("_")+1);
                    		var $thead = $('thead',self.hDiv);
                    		var thDiv = $thead.find("th[abbr='" + name + "']").children("div");
                    		if(thDiv != undefined && thDiv[0] != undefined){
	                    		//var childDiv = thDiv.children();
								var text = thDiv.text();
	                    		//if(text.indexOf("-") > 0){
	                    		...
	                    		thDiv[0].firstChild.nodeValue = newText;
	                    		//thDiv.append(childDiv);
                    		}
        ...
        _appendTdClick:function(){
        	...
        	for (var i=0,len=cms.length;i<len;i++) {
                ...
                var name = cm.name;
                if(cm.onRowClick) {
                	this.element.find("td[abbr='" + name + "']").each(function(rowIndex,one){
                		var rowData = self._getRowData(rowIndex);
                		$(one).bind("click", function(){
                			cm.onRowClick(rowIndex,rowData,null)
                			}
                		);
                	});
                }
                if(cm.onRowDblClick) {
                	this.element.find("td[abbr='" + name + "']").each(function(rowIndex,one){
                		var rowData = self._getRowData(rowIndex);
                		$(one).bind("dblclick", function(){
                			cm.onRowDblClick(rowIndex,rowData,null)
                			}
                		);
                	});
                }
        ...
        _addData:function(data){
            ...
            //刷新分页条
            if(!op.ifAjaxCount || op.isOnlyCount) {
	            this._buildPager();
            } 
            if(op.isOnlyCount) {
            	op.isOnlyCount = false;
            } else {
	            this._renderDatas();
            }
        },
        ...
        _renderDatas:function(){
            ...
            $.each(rows,function(i, rowData) {
                ...
                bodyContent.push("<tr _grid_row_id="+(self._guid++)+" class='om-grid-row " + rowCls + " " + eleId + "'>");
           	...
           	$("div[class='innerCol ']").each(function() {
        ...
        _buildRowCellValues:function(colModel,rowData,rowIndex){
            ...
            for(var i=0;i<len;i++){
                ...
                if(c.flowType == 1) {
                   	//预处理  
                   	var $tdHtml = $('<div>' + v + '</div>');
                   	
                   	//判断是添加 审批和申请审批的操作
                   	if($tdHtml != null && $tdHtml != undefined && op.flowLimit) {
                   		//如果有审批限制 则只有无审核 和 审核不通过可以进行 修改等 其他操作
                   		if(rowData.XQY_FR_DATA != null) {
                 			$tdHtml.append('<a class="check_edus_list" title="审批" onclick="onXqyFlowCheck(\'' + rowData.FLOW_LMF_REAL_ID + '\',\'' + rowData.TABLE_ID + '\',\'' + rowData.XQY_FR_DATA + '\',\'' + rowData.NEXT_USER_ID + '\')"></a>');
                   		}
                   		//此处可新增 限制操作 如果审核通过或不通过则 不可进行其他操作 
                   		
	                }
                 	flag = true;
	                 
	                 //判断是否添加动态 操作按钮的操作
                   	if((typeof autoPageButton_lmf) != undefined && autoPageButton_lmf.length > 0) {
						$tdHtml.append($(autoPageButton_lmf));             		
                   	}
                }
                if(flag) {
                	values[i]=$tdHtml.html();
                } else {
	                values[i]=v;
                }
                v = null;
        ...
        _bindSelectAndClickEnvent:function(){
            ...
            if(!this.options.singleSelect){ //可以多选
                ...
                this.tbody.delegate('tr.om-grid-row.' + eleId,'click',function(event){
                ...
                //行双击
                this.tbody.delegate('tr.om-grid-row.' + eleId,'dblclick',function(event){
                    var row=$(this),index=self._getRowIndex(row);
                    if(row.hasClass('om-state-highlight')){ //已选择
                        //do nothing
                    }else{
        ...
        _rowSelect:function(index, noBackSelectFun){
             var el=this.element,
                op=this.options,
                tbody=$('tbody',el),
            ...
            var _ignoreIndex = op.allSelectedIgnorePageIndex;
            var tmpRowData = this._getRowData(index);
            var key = el.data("key");
            var ifExits = true;
            var removeIndex = -1;
            for(var j = 0; j < _ignoreIndex.length; j++) {
         		var oneIgnoreIndex = _ignoreIndex[j];
         		if(tmpRowData[key] == oneIgnoreIndex[key]) {
         			ifExits = false;
         			removeIndex = j;
         		}
          	}
          	if(ifExits) {
          		op.allSelectedIgnorePageIndex.push(tmpRowData);
          	} else if(removeIndex >= 0) {
          		op.allSelectedIgnorePageIndex[removeIndex] = tmpRowData;
          	}
          	if(!noBackSelectFun) {
            	this._trigger("onRowSelect",null,index,tmpRowData);
            }
        },
        
        rowDeSelect:function(index){
        	this._rowDeSelect(index);
        },
        ...
        _rowDeSelect:function(index){
            ...
            for(var j = 0; j < _ignoreIndex.length; j++) {
          	...
          	if(ifExits && removeIndex != -1) {
          		op.allSelectedIgnorePageIndex.splice(removeIndex,1);
          	}
		...
		_getTrs:function(){
			return this.tbody.children("tr.om-grid-row:not([_delete]=true)");		
		},
		//获取所有真正的行，此方法一样可以兼容其它插件。
		getTrLength:function(){
			return this.tbody.children("tr.om-grid-row:not([_delete]=true)").length;		
		},
        //public methods
        getTr:function(index) {
        	return this.tbody.children("tr.om-grid-row:not([_delete]=true):eq(" + index + ")");		
        },
        /**
        ...
        reload:function(page){
            ...
            //获取选中值
            var selectedArr = this.getSelections();
            this.reloadChecked = selectedArr;
         ...
         * 设置忽略分页选中时的初始选中值
         */
        setAllSelectedIgnorePageIndex:function(json){
        	var self=this;
        	self.options.allSelectedIgnorePageIndex = json;
        },
        /**
         ...
         * 
         */
        setSelections:function(indexes, noBackSelectFun){
            var self=this;
            if(this.options.pluralSelectIgnorePage || this.options.singleSelect) {
            	if(indexes != null) {
	           		this.options.allSelectedIgnorePageIndex = indexes;
	           	}
	           	var _ignoreIndex = this.options.allSelectedIgnorePageIndex;
	           	if(_ignoreIndex != null) {
		           	var tmpDataSelf = self.getData();
		           	if(tmpDataSelf != undefined) {
		            	var rowsData = tmpDataSelf.rows;
		            	var $trs = self._getTrs();
		            	var selectedTrs = $trs.filter('.om-state-highlight');
		            	selectedTrs.each(function(index , tr){
		            		var trIndex = rowsData[$trs.index(tr)];
		            		self._rowDeSelect(trIndex);
		            	});
		            	var key = self.element.data("key");
		        		if(key != null) {
			            	$(rowsData).each(function(i, one){
			            		var flag = false;
			            		for(var j = 0; j < _ignoreIndex.length; j++) {
				            		if(one[key] == _ignoreIndex[j][key]) {
				            			flag = true;
				            			_ignoreIndex[j] = one;
				            		}
			            		}
	            ...
	            if(!$.isArray(indexes) && indexes != null){
	                indexes=[indexes];
	            }
	            ...
	            $(indexes).each(function(){
	                self._rowSelect(this, noBackSelectFun);
	            });
	            self._refreshHeaderCheckBox();
            }
        },
         ...
         * 
         */
        getSelections:function(needRecords, ifIgnore){
            ...
            var self=this,
            	...
            	result=[];
            //单选时默认为false
            ifIgnore = self.options.singleSelect?false:ifIgnore;
            if(needRecords){
            	if(self.options.pluralSelectIgnorePage && ifIgnore) {
            		result = self.options.allSelectedIgnorePageIndex;
            	} else {
	            	var rowsData = self.getData().rows;
	            	selectedTrs.each(function(index , tr){
	            		rowsData[$trs.index(tr)].indexTr=$trs.index(tr);
    ...
    $.om.lang.omGrid = {
        ...
        pageStat:'符合您条件的数据共{total}条，当前页面显示为{from}-{to}条数据'
    };
})(jQuery);/*
    ...
    $.omWidget('om.omMenu', {
        ...
        show : function(triggerEle){
            ...
            if( options.contextMenu ){
            	options.nowClickId = $(triggerEle.target).closest("td").attr("id");
        ...
        _create:function(){
            ...
            if(options.zIndex > 0) {
            	$ele.css('z-index',options.zIndex);
            }
            if(source) {
                if(source != 'local'){
        ...
        _bindLiEvent : function(li){
            ...
            $(li).bind("mouseenter.menuItem",function(){
                ...
                if(options.onSelect){
                	self._trigger("onSelect",event,item, options);
                }
            });
        },
...
(function( $, undefined ) {
    ...
    var _show = function(config){
        ...
        var cmQd=(config.buttonName!=null && config.buttonName!=undefined && config.buttonName.length>1)?config.buttonName[0]:'确定';
        var cmQx=(config.buttonName!=null && config.buttonName!=undefined && config.buttonName.length>=2)?config.buttonName[1]:'取消';
        switch (config.type) {
            case 'confirm':
                buttonSet.html('<button id="confirm">'+cmQd+'</button><button id="cancel">'+cmQx+'</button>');
                if($.fn.omButton){
...
(function( $, undefined ) {
    ...
    $.omMessageTip = {
        ...
        animation_prompt_fun(config.content);//ztx更改提示消息样式
...
(function(){
    ...
    $.omWidget('om.omTabs', {
        ...
        _init : function() {
            ...
            this._afterRender();
            this._buildEvent();
            if ($(this.element).find(".om-tabs-headers").children('ul').hasClass('cus_wdtabul')) {
        	//	$(this.element).find(".om-tabs-headers").children('ul').css('height','60px'); //ztx取消标签卡的高度设置。
        	} else {
        		$(this.element).find(".om-tabs-headers").children('ul').css('height','29px');
        		$(this.element).find(".om-tabs-headers").css('height','30px');
        		$(this.element).find(".om-tabs-headers").children('ul').children("li").find("a").css('line-height','29px');
        	}
        },
        ...
        _makeSketch : function() {
            this.element.addClass('om-tabs om-widget om-widget-content om-corner-all')
                ...
                .find('>ul').wrap('<div class="om-tabs-headers om-helper-reset om-helper-clearfix om-widget-header om-corner-all" style="overflow:hidden;"/>');
    	...
    	_collectItems : function() {
            ...
            $tmpUl = $self.find('>div.om-tabs-headers ul');
            
            if($lis.length > 0) {
		        $('<span class="left-placeholder"></span>').insertBefore($lis.eq(0));
		        $('<span class="right-placeholder"></span>').insertAfter($lis.eq($lis.length - 1));
            } else {
            	$('<span class="left-placeholder"></span>').appendTo($tmpUl);
            	$('<span class="right-placeholder"></span>').appendTo($tmpUl);
            }
	...
	//            omtabs.children(':first').css('width',options.width);
		...
		_renderHeader : function() {
	        ...
	        var aHeight = options.tabHeight;//$lis.find('a.om-tabs-inner').height();
	     /*   $headers.find('ul').css({
	            'height' :  aHeight-1 ,
		...
		_renderBody : function() {
	        ...
	        if (options.height !== 'auto') {
	            ...
	            $panels.css('height', omtabsHeight - headersHeight + 30);
	        }
    	...
    	_add : function(config/*title, content, url, closable , index, tabId, activateNew */) {
	        ...
	        var $anchor = $('<a class="om-tabs-inner"></a>').html(config.title).attr({
	                href : '#' + (config.href?config.href:tabId),
	                tabId : tabId
	            }).css({
	             //   width : options.tabWidth,
	             //   height : options.tabHeight  //ztx取消每个单独标签的高和宽度设置
	            }).appendTo($nHeader);
	        ...
	        this._checkScroller() && this._enableScroller();
	        if($self.find('>div.om-tabs-panels').children().length > 0) {
	        	...
	        	items[config.index].addClass("om-state-nobd").parent().appendTo($self.find('>div.om-tabs-panels'));
	        }
	        this._purgeEvent();
	        this._buildEvent();
	        ...
	        //从新加载内容触发 子页面中的document加载方法  暂时这样修改
	        if(config.content != undefined) {
	        	this._setDataSource({index:config.index, content:config.content});
	        }
    	},
		...
		if (command) {
			...
			case "add":
				...
				if (argument.messages)
					...
					$(element.form).lmfValidate("setOption","rules",settings.rules);
					$(element.form).lmfValidate("setOption","messages",settings.messages);
					$(element.form).lmfValidate("appendValidate");
				break;
			case "remove":
				if (!argument) {
...
$.extend($.validator, {
	...
	prototype: {
		...
		checkForm: function() {
			...
			for ( var i = 0, elements = (this.currentElements = this.elements()); elements[i]; i++ ) {
				//判断该节点是否是隐藏，隐藏则不验证
				if(($(elements[i]).is(':hidden') && !$(elements[i]).parent().hasClass('om-combo')  && $(elements[i]).attr('fieldRole')!=18 && $(elements[i]).attr('fieldRole')!=19) 
				|| $(elements[i]).parent().is(':hidden')
		...
		element: function( element ) {
			if(typeof element == "string") {
				...
				for(var i = 0; i < tmp.length; i++) {
					if(tmp[i].length > 0) {
						tmpElement = this.clean( tmp[i] );
						this.lastElement = tmpElement;
						this.prepareElement( tmpElement );
						this.currentElements = $(tmpElement);
						var result = this.check( tmpElement );
						if ( result ) {
							delete this.invalid[tmpElement.name];
						} else {
							this.invalid[tmpElement.name] = true;
						}
	...
	methods: {
		...
		required: function(value, element, param) {
			...
			case 'input':
				if ($(element).parent().hasClass('om-combo')) {
					var value = $(element).omCombo('value');
    				if(value != null && value.length > 0) {
    					return true;
    				} else {
    					return false;
    				}
				}  else if ($(element).attr('fieldRole')==18 || $(element).attr('fieldRole')==19) {
					if ($(element).data('fileId')!=undefined && $(element).data('fileId')!=null && $(element).data('fileId')!='') {
						return true;
					}
					return false;
		...
		remote: function(value, element, param) {
			...
			var tmpThis = this;
			//加一个特殊判断 如果 url请求地址已form结尾 则 获取当前元素的form表单的 所有值传递至后台
			if(typeof param == "string" && param.endWith("form")) {
				//是客户化的后台唯一性验证
				param = param.substring(0,param.length-4);
				data = $(element).closest("form").lmfSerialize();
				data += "&inputNameValue=" + value;
			} else {
				param.url = param.url.substring(0,param.url.length-4);
				data = $(element).closest("form").lmfSerialize();
				data += "&inputNameValue=" + value;
			}
			$.ajax($.extend(true, {
				...
				success: function(response) {
					...
					if(response == 'true' || response == 'false') {
						...
						if (response == 'true') {
							 ...
							 * 2016.1.26  韩炜  将此代码改为 var message = validator.defaultMessage( element, "remote" );
							 *  在remote的返回结果提示
							 * */
...
(function($) {
	/**
      ...
      * @param p 标准config对象:{}
      */
    function ztxFun(ztxSet){
	//	ztxSet = {self : self, delta : delta, over : ui.position, oOver : ui.originalPosition}
	//	ztxSet.over//拖动滚动条后获取一直变化有效范围内的数值。
	//	ztxSet.oOver//拖动滚动条后获取滚动条原位置的定位数值。
		var self = ztxSet.self,
		ztxscroll = {};
		//获取滚轮的上、下的滚动方向。
		ztxscroll.delta = ztxSet.delta ? ztxSet.delta : 0;
		//拖动滚动条的上下，左右具体位置。
		ztxscroll.overTop = ztxSet.over ? ztxSet.over.top || 0 : 0;
		ztxscroll.overLeft = ztxSet.over ? ztxSet.over.left || 0 : 0;
		//滚轮每次滚动像素距离。
		ztxscroll.differ = (ztxscroll.delta / 120 * 30) || 0;
		//模拟滚动条右侧。
		ztxscroll.DivRight = self._vScrollbar;
		//模拟滚动条底部。
		ztxscroll.DivBottom = self._hScrollbar;
		//被滚动的元素。
		ztxscroll.Con = self.element;
		//被滚动元素高度，宽度。
		ztxscroll.ConHeight = ztxscroll.Con.outerHeight();
		ztxscroll.ConWidth = ztxscroll.Con.outerWidth();
		//被滚动元素距离顶部,左侧距离。
		ztxscroll.ConTop = ztxscroll.Con.offset().top;// + self._getInt($elem.css("border-top-width")) + self._getInt($elem.css("padding-top")) + self._getInt($elem.css("padding-top-width"));
		ztxscroll.ConLeft = ztxscroll.Con.offset().left;// + self._getInt($elem.css("border-left-width")) + self._getInt($elem.css("padding-left")) + self._getInt($elem.css("padding-left-width"));
		//模拟滚动条的上下，左右最大滚动距离。
		ztxscroll.DivDistanceTop = ztxscroll.ConHeight - ztxscroll.DivRight.outerHeight();
		ztxscroll.DivDistanceLeft = ztxscroll.ConWidth - ztxscroll.DivBottom.outerWidth();
		//内容上下，左右能够滚动的最大滚动距离。
		ztxscroll.ConDistanceTop = ztxscroll.Con[0].scrollHeight - ztxscroll.ConHeight;
		ztxscroll.ConDistanceLeft = ztxscroll.Con[0].scrollWidth - ztxscroll.ConWidth;
		//滚轮控制内容，模拟滚动条上下滚动的距离。
		ztxscroll.ConTopWheel = parseInt(ztxscroll.Con.scrollTop() + ztxscroll.differ);
		ztxscroll.divTopWheel = parseInt((ztxscroll.ConTopWheel / ztxscroll.ConDistanceTop) * ztxscroll.DivDistanceTop + ztxscroll.ConTop);
		//鼠标拖动后模拟滚动条上下,左右滚动的距离。
		ztxscroll.divTopOver = parseInt((ztxscroll.Con.scrollTop() / ztxscroll.ConDistanceTop) * ztxscroll.DivDistanceTop + ztxscroll.ConTop) || ztxscroll.ConTop;
		ztxscroll.divLeftOver = parseInt((ztxscroll.Con.scrollLeft() / ztxscroll.ConDistanceLeft) * ztxscroll.DivDistanceLeft + ztxscroll.ConLeft) || ztxscroll.ConLeft;
		//拖动设置内容上下，左右滚动距离。
		ztxscroll.overConTop = parseInt(((ztxscroll.overTop - ztxscroll.ConTop) / ztxscroll.DivDistanceTop) * ztxscroll.ConDistanceTop);
		ztxscroll.overConLeft = parseInt(((ztxscroll.overLeft - ztxscroll.ConLeft) / ztxscroll.DivDistanceLeft) * ztxscroll.ConDistanceLeft);
		//定义当前的滚动条宽度。
		ztxscroll.options = self.options.thick;
		ztxscroll.parameter = self.options;
       	return ztxscroll;
	};
	$.omWidget("om.omScrollbar" , {
		...
		_create : function(){
			var ops = this.options;
			this._vScrollbar = $("<div class='om-widget om-scrollbar om-corner-all scroll_Height'></div>").width(ops.thick).appendTo("body").hide();
			this._vScrollbar.type = "v";
			this._hScrollbar = $("<div class='om-widget om-scrollbar om-corner-all scroll_Width'></div>").height(ops.thick).appendTo("body").hide();
			this._hScrollbar.type = "h";
		},
		
		_init : function(){
			if($(this.element).parent().closest('.ztxScroll').length <= 0.1 && !$(this.element).hasClass('ztxScroll')){
				$(this.element).addClass('ztxScroll');
		...
		destroy : function(){
			...
			$(this.element).removeClass('ztxScroll');
		}, 
		...
		_buildEvent : function(){
			...
			function openTreu(){
				var ztxSet = {self : self},
		        ztxscroll = new ztxFun(ztxSet);
				$(bars).each(function(index , $bar){
					$bar._hover = true;
					if('v' === $bar.type){self._resize(ztxscroll.DivRight);}else if('h' === $bar.type){self._resize(ztxscroll.DivBottom);};
					clearTimeout($bar._timer);
					if($bar._enable){$bar.fadeIn("fast");};
				});
			};
			function openFalas(){
				$(bars).each(function(index , $bar){
			...
			$elem.bind('click',function(){if(eval(this.scrollHeight-1) >= eval($(this).height())){openTreu();}else{openFalas();};});
			$(bars).each(function(index , $bar){
				$bar._hover = false;
				$bar._enable = true;
				var type = $bar.type;
				...
				$bar.omDraggable({
					...
					*/
						var ztxSet = {self : self, over : ui.position, oOver : ui.originalPosition},
						ztxscroll = new ztxFun(ztxSet);//ztx鼠标拖动右侧或底部滚动条后的页面及模拟滚动条定位
						type == 'v'?$elem.scrollTop( ztxscroll.overConTop ):$elem.scrollLeft( ztxscroll.overConLeft );
					}
				}).hover(function(){
					$(bars).each(function(index , $bar){
						$bar._hover = true;
			...
			if($.browser.version < 12){eventName = "mousewheel"};
    	...
    	*/
    		var ztxSet = {self : self, delta : delta},
	        ztxscroll = new ztxFun(ztxSet),
	        divTop = ztxscroll.divTopWheel,
	        ConTop = ztxscroll.ConTopWheel;
			self._resize(ztxscroll.DivRight);
			self._resize(ztxscroll.DivBottom);
        	if(ConTop <= 0){
        		ConTop = 0;
        		divTop = ztxscroll.ConTop;
        	}else if(ConTop >= ztxscroll.ConDistanceTop){
        		ConTop = ztxscroll.ConDistanceTop;
        		divTop = ztxscroll.DivDistanceTop + ztxscroll.ConTop;
        	};
	        	self._vScrollbar.css("top" , divTop );
	        	self.element.scrollTop( ConTop );//重新编写鼠标滚轮变化后的页面及模拟滚动条定位。
	        event.preventDefault();
		},
		...
		_resize : function($bar){
			...
			if('v' === type){
				...
				//	$bar.outerHeight( this._getInt(h*$elem.innerHeight()/size) );
		        	var ztxSet = {self : this},
		       		ztxscroll = new ztxFun(ztxSet);
					var barHeight = this._getInt(h*$elem.innerHeight()/size), rightDistance = 0;
					if(barHeight <= 100){barHeight = 100;};
					if($elem.height() <= 110){barHeight = $elem.height() - 10};
					if(ztxscroll.parameter.rightDistance){rightDistance = ztxscroll.parameter.rightDistance};
					$bar.outerHeight( barHeight );//ztx鼠标滑入内容区域，右侧滚动条的高度及定位。
					$bar.css({
						"left" : offset.left + ow - $bar.outerWidth() - br + parseInt(rightDistance),
						"top" : ztxscroll.divTopOver//offset.top + bt + pt  + this._getInt($elem.scrollTop() * h/ size)
					});
				}else{
					$bar._enable = false;
				}
				...
				//	$bar.outerWidth( this._getInt((pl+w)*w/size) );
					var ztxSet = {self : this},
		       		ztxscroll = new ztxFun(ztxSet);
					var barWidth = this._getInt((pl+w)*w/size), bottomDistance = 0;
					if(barWidth <= 100){barWidth = 100;};
					if($elem.width() <= 110){barWidth = $elem.width() - 10};
					if(ztxscroll.parameter.bottomDistance){bottomDistance = ztxscroll.parameter.bottomDistance};
					$bar.outerWidth( barWidth );//ztx鼠标滑入内容区域，底部滚动条的宽度及定位。
					$bar.css({
						"left" : ztxscroll.divLeftOver,//offset.left + bl + pl + this._getInt($elem.scrollLeft()*w/ size),
						"top" : offset.top + oh - $bar.outerHeight() - bb + parseInt(bottomDistance)
					});
				}else{
					$bar._enable = false;
				}
    ...
    $.omWidget('om.omButtonbar', {
        ...
        _init : function(){
            ...
            $.each(options.btns, function(index, props) {
            	if(!props.separtor){
    				...
    				var button = $("<button type=\"button\"></button>")
    				    .attr('id',btnId)
    				    .attr('title',props.title==undefined?'':props.title)
    					.appendTo(element);
    				if ($.fn.omButton) {
	     ...
	     * 如果tab页签总宽度较大，则显示scroll并返回true；否则删除scroll并返回false。
     	...
     	_checkScroller : function() {
	        ...
	        var $ul = $self.find('>div.om-buttonbar');
	        var totalWidth = 4;  // padding
	        $ul.children().each(function() {
	            //计算一个li占用的总宽度
	            totalWidth += $(this).outerWidth(true);//sub element's width
...
(function($) {
    $.omWidget.addInitListener('om.omGrid',function(){
        var self = this,
            ...
            cm = ops.colModel,
            $thead = this.thead,
            rdpFun = ops.rowDetailsShowFun;//行详情 展开时的点击方法
            widgetConHei();//ztx设置框架的高度。
        if( !(rdp && cm.length>0) ){
            return;
        }
        ...
        this.tbody.delegate('td.expenderCol >div','click',function(event){
            ...
            if($next.hasClass('rowExpand-rowDetails')){ //已经构造过了，直接显示/隐藏
            	...
            	//隐藏掉其他所有的展开的
            	tmpTbody.children("tr").children('td.expenderCol').children("div").each(function(i, one){
            		var $tmpNext = $(one).closest('tr').next('tr');
            		if($tmpNext.hasClass('rowExpand-rowDetails')) {
            		
            			$tmpNext.toggle();
                 		$(one).toggleClass('rowExpand-expanded').parent()
            			.attr('rowspan' , $(one).hasClass('rowExpand-expanded')? 2:1);
						
						if ($tmpNext.is(':hidden')) {
							$tmpNext.remove();
						}
					}
            	});
                ...
                var $tmpTr = $('<tr class="rowExpand-rowDetails"><td colspan="'+(colCount-1)+'"><div class="rowExpand-rowDetails-content">'+rowDetails+'</div></td></tr>');    
                $row.after($tmpTr);
	            _this.toggleClass('rowExpand-expanded').parent()
	            	.attr('rowspan' , _this.hasClass('rowExpand-expanded')? 2:1);
				_resizeRowExpander.call(self);
                rdpFun(rowData, rowIndex);
            }
            
           
            return false; //不触发onRowSelect和onRowClick事件
            // 修改列表下展开内嵌列表内容div高度问题
			var widgetContent = $('.om-grid.om-widget.om-widget-content'),
			widgetConPrev = widgetContent.prev(),
			widgetConH = widgetContent.closest('body').height()-widgetConPrev.outerHeight()-2-widgetConPrev.prev().outerHeight();
			$("table[id^='listTableId']").children(".bDiv").each(function(i,one){
				widgetContent.height(widgetConH);
				$(one).height(widgetConH-widgetContent.children('.hDiv').outerHeight()-widgetContent.children('.pDiv').outerHeight());
			});
        });
    });
    ...
    function _resizeRowExpander(){
    	...
    	//计算行详情的宽度
    	var $ht = this.hDiv.find("table:first");
    	var width = $ht.width() - $ht.find("th.expenderCol").outerWidth() - parseInt($rowExpander.find("> td").css("border-right-width"));
		var outtbwidth = $rowExpander.closest(".bDiv").width();
		var firsttdwidth = $ht.find("th.expenderCol").width();
    	$rowExpander.each(function(){
    		$(this).find(".rowExpand-rowDetails-content").outerWidth(outtbwidth-firsttdwidth-15);
    	});
    }
})(jQuery);/*
			...
			_buildTableHead : function(){
	            ...
	            for(var i=0,row=cm.length; i<row; i++){
	            	...
	            	if(i == 0){//行号和多选框标题列永远在第一行
	            		...
	            		if(!op.singleSelect){
	            			content.push("<th class='checkboxCol data-header-"+row+"' align='center' axis='checkboxCol' rowspan="+row+"><div class='checkboxheader' style='text-align:center;width:17px;'>");
	            			if(op.checkAllDefault) {
	            			    content.push("<span class='checkbox'/>");
	            			}
	            			content.push("</div></th>");
	            		}
	            	}
...
jQuery.extend({
    // 不进行后台ajax操作 记录需要操作的url 和请求地址  添加审核信息 当审核通过时进行 调用添加操作
    toFlowAddAjax:function (json) {
      	//获取url 信息 和参数信息   获取审核流程id  和审核流程备注信息
      	var flowJquery = $("#addFlow_lmfFrm");
      	if(flowJquery != null && flowJquery.length > 0) {
	      	var pageJquery =  flowJquery.closest(".om-dialog-content");
	      	pageJquery.find(":input").each(function(){
				$(this).attr("hidValue", $(this).val());
			})
			pageJquery.find(":file").closest("li").find("object").remove();
			var $tmpPage= $("<div></div>");
			pageJquery.children("div,ul,form").each(function(){
				$tmpPage.append($(this).html());
			});
	      	var htmlStr = $tmpPage.html();
	      	
	      	var shFrmCom = flowJquery.serialize();
	      	//获取参数 
	      	var url = json.url;
	      	if(url.indexOf("?") == -1) {
	      		url = url + "?";
	      	...
	      	if(typeof prevData == "json") {
	      		prevData = $.param(prevData);
	      	}
	      	$.ajax({
				type: "post",
				url: "autoUpdatePage_updatePage.do",
				data: shFrmCom + "&html=" + encodeURI(htmlStr) + "&url=" + url + "&" + prevData,
				success: function(data){
					...
					$.omMessageTip.show({
				    	title : '操作成功',
				    	content : '保存数据成功,请去审核页面进行相应的数据查看,审核通过后会保存成功！',			    	
				        timeout : 3000
	        		});
	                pageJquery.omDialog("closeAndRemove");
				},
				 error: function(){}
			});
		} else {
    ...
    // 封装lmfSerialize  进行参数的获取和拼接
jQuery.fn.lmfSerialize=function () {
   	//第几层
   	var index = 0;
   	...
   	var json = {};
   	//获取块展示的表格对象
   	var	firstTable = $(this).find('.blockTableDiv').children("table");
   	if (firstTable.length<=0) {
   		firstTable = $(this).children().children("table");
   	}
   	var pageType = $(this).attr('type');//页面类型
   	if (pageType!=null && pageType!=undefined && pageType==6) {//批量添加特殊处理
   		firstTable =  $(this).find("table[name='addTab_"+($(this).attr('name').substring(7))+"']");
   	}
   	$.tableSerialize(json, firstTable, index);
   	var str = "";
   	for(var key in json){
    ...
    str+="&ifCleanInnerData="+$("input[name='ifCleanInnerData'").val() + "&";
   	return str;
}
 jQuery.extend({   
    //递归调用 拼接json
    tableSerialize:function(json, firstTable, index){
    	//获取第一层的所有的td
      	var firstTdArr = firstTable.children("tbody").children("tr").children("td").children("div,table");
      	
      	if(firstTdArr.length > 0) {
      		$(firstTdArr).each(function(i, one){
      			//获取td中的内容 判断内容类型为什么
      			var tdTable = $(one);
      			//有子表格内容 为内部对象或内部对象多值信息
      			if(tdTable.is("table") > 0) {
      				//index ++;
      				//递归增加层 继续拼接 子表格的 属性信息
      				$.tableSerialize(json, tdTable, index+1);
      			} else {
      				//为一个属性信息
      				var tdDiv = $(one);
      				var aouInputArr = tdDiv.find("[aouInput=1]");
      				if(aouInputArr.length > 0) {
      				    for(var i = 0; i < aouInputArr.length; i ++) {
      				        var aouInput = $(aouInputArr[i]);
		      				//取出字段类型  
		      				var fieldRole = aouInput.attr("fieldRole");
		      				//取出字段的name属性
		      				var fieldName = aouInput.attr("name");
		      				//截取 获得 tableId 和 pageId 
		      				var fieldNameArr = fieldName.split("_");
		      				var tableId = fieldNameArr[1];
		      				//为内部对象属性 没有特殊操作 正常获取data属性
		      				if(fieldRole == 20 || fieldRole == 22) {
		      					var prevName = aouInput.attr("prevName");
		      					if (aouInput.parent().hasClass('om-combo')) {
		      						var value = aouInput.omCombo('value');
		      						if(typeof value == "string") {
			      						if(eval("json.t" + index + "_" + prevName + "==undefined")) {
			      							eval("json.t" + index + "_" + prevName + "=[];");
			      						}
			      						eval("json.t" + index + "_" + prevName + "[json.t" + index + "_" + prevName + ".length] = value;");
			      					}
		      					} else {
		      						var fieldJson = aouInput.data("json");
		      						var id = 0;
		      						if(fieldJson != undefined && fieldJson.length > 0) {
			      						if(fieldJson.length > 0) {
			      							id = eval("fieldJson[0].T_" + tableId + "_0");
			      						} else {
			      							id = "";
			      						}
			      						//json key为 传至后台的name  json的value为 传至后台的value
			      						if(eval("json.t" + index + "_" + prevName + "==undefined")) {
			      							eval("json.t" + index + "_" + prevName + "=[];");
			      						}
			      						eval("json.t" + index + "_" + prevName + "[json.t" + index + "_" + prevName + ".length] = id;");
			      					}
			      				}
		      				} else if(fieldRole == 21) {
		      					//内部对象多值  重要是获取 子表格的记录个数
		      					var value = 0;
		      					var tmpChildTable = aouInput.closest("tr").next("tr").children("td").children("#" + fieldName + "_childTable:first");
		      					if(tmpChildTable.length == 0) {
		      					    var tmpJson = aouInput.data("json");
		      					    var tmpname21 = aouInput.attr("name");
		      					    tmpname21 = tmpname21.substring(3);
		      					    //取json的第一个属性为id 传至后台
		      					    var tmpValue = [];
		      					    var tmpIdForUpd = [];
		      						if (aouInput.parent().hasClass('om-combo')) {
		      							...
		      							if(typeof value == "string") {
			      							tmpJson = value.split(',');
			      							if(tmpJson != null && tmpJson.length > 0) {
		      					        		for(var lmf = 0; lmf < tmpJson.length; lmf++) {
		      					        			tmpValue[tmpValue.length] = tmpJson[lmf];
			      					            	tmpIdForUpd[tmpIdForUpd.length] = tmpJson[lmf];
										   	 	}
									    	}
			      						}
		      						} else {
		      					    	if(tmpJson != null && tmpJson.length > 0) {
		      					        	for(var lmf = 0; lmf < tmpJson.length; lmf++) {
			      					      	  	var tmpI = 0;
			      					          	var tmpOneJson = tmpJson[lmf];
			      					        	for (var key in tmpOneJson) {
			      					            	if(key.indexOf("T_") == 0 && key.indexOf("_0") == key.length-2 && key != "T_" + tmpname21.substring(0,tmpname21.indexOf("_")) + "_0") {
					      					    	    tmpValue[tmpValue.length] = tmpOneJson[key];
					      					    	    break;
					      					    	}
			      					           	}
			      					         	tmpIdForUpd[tmpIdForUpd.length] = (tmpOneJson.upId==undefined?"":tmpOneJson.upId);
										   	 }
									    }
		      					    }
									if(eval("json.t" + index + "_" + fieldName + "_dz==undefined")) {
	                                    eval("json.t" + index + "_" + fieldName + "_dz=[];");
	                                }
								    eval("json.t" + index + "_" + fieldName + "_dz[json.t" + index + "_" + fieldName + "_dz.length] = tmpValue.length;");
									//拼接下一层的 需要替换的 子查询sql的key
									var dialogFieldId = aouInput.attr("dialogFieldId");
									var nextFieldname = fieldName.substr(0, fieldName.lastIndexOf("_")) + "_" + dialogFieldId;
									if(eval("json.t" + (index+1) + "_" + nextFieldname + "==undefined")) {
                                       eval("json.t" + (index+1) + "_" + nextFieldname + "=[];");
                                    }
                                     //如果是修改页面 需要拼接当前的 原本的id 信息传至后台中
                                    if(eval("json.t" + (index+1) + "_" + fieldName.substr(0, fieldName.lastIndexOf("_")) + "==undefined")) {
                                       eval("json.t" + (index+1) + "_" + fieldName.substr(0, fieldName.lastIndexOf("_")) + "=[];");
                                    }
                                    for(var lmfI = 0; lmfI < tmpValue.length; lmfI++) {
                                       eval("json.t" + (index+1) + "_" + nextFieldname + "[json.t" + (index+1) + "_" + nextFieldname + ".length] = tmpValue[lmfI];");
                                    }  
                                    for(var lmfI = 0; lmfI < tmpIdForUpd.length; lmfI++) {
                                       eval("json.t" + (index+1) + "_" + fieldName.substr(0, fieldName.lastIndexOf("_")) + "[json.t" + (index+1) + "_" + fieldName.substr(0, fieldName.lastIndexOf("_")) + ".length] = tmpIdForUpd[lmfI];");
                                    }  
		      					} else {
			      					if(aouInput.length > 0) {
			      						//value = tmpChildTable.children("tbody").children().length;
			      						value = 0;
			      						tmpChildTable.children("tbody").children().each(function(i, tttOne){
			      							if($(tttOne).find("table").length == 0) {
			      								value ++;
			      							}
			      						});
			      					...
			      					eval("json.t" + index + "_" + fieldName + "_dz[json.t" + index + "_" + fieldName + "_dz.length] = value;");
			      					var tmpJson = aouInput.data("json");
			      					var tmpname21 = aouInput.attr("name");
				      				var tmpValue = [];
		      					    if(tmpJson != null && tmpJson.length > 0) {
		      					...
		      					if(typeof value == "string") {
			      					if(fieldRole == 17) {
			      						if(eval("json.t" + index + "_" + fieldName + "_dicMany==undefined")) {
				      						eval("json.t" + index + "_" + fieldName + "_dicMany=[];");
				      					}
			      						eval("json.t" + index + "_" + fieldName + "_dicMany[json.t" + index + "_" + fieldName + "_dicMany.length] = value;");
			      					} else {
			      						if(eval("json.t" + index + "_" + fieldName + "==undefined")) {
				      						eval("json.t" + index + "_" + fieldName + "=[];");
				      					}
			      						eval("json.t" + index + "_" + fieldName + "[json.t" + index + "_" + fieldName + ".length] = value;");
			      					}
			      				}
		      				} else if(fieldRole == 18 || fieldRole == 19) {
		      					//文档类型
		      					var fileId = aouInput.data("fileId");
		      					if(fieldRole == 19) {
		      						if(eval("json.t" + index + "_" + fieldName + "_file_many==undefined")) {
			      						eval("json.t" + index + "_" + fieldName + "_file_many=[];");
			      					}
		      						eval("json.t" + index + "_" + fieldName + "_file_many[json.t" + index + "_" + fieldName + "_file_many.length] = (fileId==undefined?'':fileId);");
		      					} else {
		      						if(eval("json.t" + index + "_" + fieldName + "==undefined")) {
			      						eval("json.t" + index + "_" + fieldName + "=[];");
			      					}
		      						eval("json.t" + index + "_" + fieldName + "[json.t" + index + "_" + fieldName + ".length] = (fileId==undefined?'':fileId);");
		      					}
		      				} else if(fieldRole == 2){
		      					//富文本
		      					var editorContent = aouInput.data("editorValue");
	      						if(eval("json.t" + index + "_" + fieldName + "_editor==undefined")) {
		      						eval("json.t" + index + "_" + fieldName + "_editor=[];");
		      					}
		      					var hidValue = aouInput.attr("hidValueId");
		      					if(hidValue != undefined && hidValue != null && hidValue.length > 0) {
			      					editorContent = aouInput.attr("hidValueId") + ":" + editorContent;
		      					} else {
		      						editorContent = editorContent;
		      					}
		      					var newEditorContent = encodeURIComponent(editorContent);
	      						eval("json.t" + index + "_" + fieldName + "_editor[json.t" + index + "_" + fieldName + "_editor.length] = newEditorContent;");
		      				} else if(fieldRole == 30){
		      					//var value = aouInput.closest("tr").attr("order");
		      					//if(value == null) {
		      						var value = (aouInput.closest("tr").prevAll().length)+1;
		      					//}
		      					if(eval("json.t" + index + "_" + fieldName + "==undefined")) {
			      					eval("json.t" + index + "_" + fieldName + "=[];");
			      				}
     ...
     *  返回值为 字段的结果值
     */
    getValueByName : function(fieldId, tableStr, funType, tableId, pageId, resultEle){
    	//根据tableStr 获取需要拼接的内容信息 分两种 一种为：主表信息  一种为外键表信息 一种为内部对象多值信息
    	if(tableStr.indexOf(tableId) == 0 && (tableStr.indexOf("_") >= tableStr.indexOf(":"))) {
    		//是主表的信息 则直接获取 字段内容信息  id和name机构为 au_tableId_pageId_fieldId
    		var nowEle = $("input[name='au_" + tableId + "_" + pageId + "_" + fieldId + "']");
    		if(nowEle.length == 0) {
    			nowEle = $("input[name$='" + "_" + fieldId + "']");
    			return nowEle.val();
    		}
    		if(nowEle.length > 0) {
    			if(resultEle != undefined && resultEle != null) {
    				//判断是否是子表格中的元素 如果是的话 获取 本行tr中的元素
    				if($(resultEle).closest("table").attr("id").indexOf("_childTable") > 0) {
	    				nowEle = $(resultEle).closest("tr").find("input[name='au_" + tableId + "_" + pageId + "_" + fieldId + "']");
	    				if(nowEle.length == 0) {
			    			nowEle =  $(resultEle).closest("tr").find("input[name$='" + "_" + fieldId + "']");
			    		}
    				}
    			}
    			var fieldRole = nowEle.attr("fieldRole");
    			var resultValue = null;
    			if(fieldRole == 16 || fieldRole == 17 || fieldRole == 23) {
    				//数据字典类型 调用获取value值的方法
    				resultValue = nowEle.omCombo('value');
    			} else if(fieldRole == 18 || fieldRole == 19) {
    				//文档类型
    				
    			} else {
    				//其他可以直接获取value的类型
    				resultValue = nowEle.val();
    			}
    			
    			if(fieldRole == 12 || fieldRole == 13){
    	...
    	} else if(tableStr.indexOf(":") > 0) { // 
    		
    		//包含：则为内部对象多值信息
    		var nowEle = $("input[trueField='" + fieldId + "']");
    		//判断 tableStr 有几层 获取对应层级的 字段对象信息
    		if(nowEle.length > 0) {
    			
	    		//根据ele元素获取 内容
	    		var tmpJson = nowEle.data("json");
	    		//获取json中的 主键属性
	    		 if(tmpJson != null && tmpJson.length > 0) {
			        //取json的第一个属性为id 传至后台
			        var tmpValue = [];
			        for(var lmf = 0; lmf < tmpJson.length; lmf++) {
				        var tmpOneJson = tmpJson[lmf];
				        for (var key in tmpOneJson) {
				            if(key.indexOf("T_") == 0 && key.indexOf("_0") == key.length-2) {
  					        	tmpValue[tmpValue.length] = tmpOneJson[key];
  					        	break;
  					        }
			            }
			    	}
			    	//如果是 内部对象或多值的主键信息 则返回 主键字符串
			    	return tmpValue.join(",");
				}
    		} else {
    			//查找是否有子查询的table然后获取 内容 根据funType进行拼接
    			var parInput = $("input[prevname*='au_" + tableId + "_" + pageId + "_" + "']");
    			if(parInput.length > 0) {
    				var parEleTableId = "";
    				$(parInput).each(function(i, tmpParInput){
	    				//获取内部对象多值的id属性  
		    			var parEleId = $(tmpParInput).attr("id");
		    			parEleTableId = parEleId.substr(0, parEleId.lastIndexOf("_"));
		    			var tmpChildTableId = parEleTableId.substring(3, parEleTableId.lastIndexOf("_"));
		    			if(parInput.length == 1 || tableStr.endsWith(tmpChildTableId) || tableStr.indexOf(tmpChildTableId)>=0) {
		    				//if(parInput.length > 1) {
			    				return false;
		    				//}
		    			}
		    		});
			    			var eleArr = $("input[name='" + parEleTableId + "_" + fieldId + "']");
			    			if(eleArr.length > 0) {
			    				//根据function的类型 判断是求和 还是合并
			    				var tmpValArr = [];
			    				$(eleArr).each(function(i,one){
			    					var tmpVal = $(one).val();
			    					tmpValArr[tmpValArr.length] = tmpVal;
			    				});
			    				if(funType == 0) {
				    				//合并
			    					return tmpValArr.join(",");
			    				} else if(funType == 1) {
			    					//去重合并
			    					return tmpValArr.join(",");
			    				} else if(funType == 2) {
			    					//最大值
			    					var maxNum = 0;
			    					for(var lmf = 0; lmf < tmpValArr.length; lmf++) {
			    					 	if(tmpValArr[lmf]*1 > maxNum) {
			    					 		maxNum = tmpValArr[lmf];
			    					 	}
			    					}
			    					return maxNum;
			    				} else if(funType == 3) {
			    					...
			    					var minNum = 0;
			    					for(var lmf = 0; lmf < tmpValArr.length; lmf++) {
			    					 	if(tmpValArr[lmf]*1 < minNum) {
			    					 		minNum = tmpValArr[lmf];
			    					 	}
			    					}
			    					return minNum;
		    					} else if(funType == 4) {
			    					...
			    					if(tmpValArr.length > 0) {
			    						return sumNum/tmpValArr.length;
			    					} else {
			    						return 0;
			    					}
			    				}
     ...
     * 获取值插件的 元素内容
     */
    getValueEle:function(fieldId, tableStr, funType, tableId, pageId, plugId){
    	//根据tableStr 获取需要拼接的内容信息 分两种 一种为：主表信息  一种为外键表信息 一种为内部对象多值信息
    	if(tableStr.indexOf(tableId) == 0  && (tableStr.indexOf("_") >= tableStr.indexOf(":"))) {
    		//是主表的信息 则直接获取 字段内容信息  id和name机构为 au_tableId_pageId_fieldId
    		var nowEle = $("input[name='au_" + tableId + "_" + pageId + "_" + fieldId + "']");
    		if(nowEle.length == 0) {
    			nowEle = $("input[name$='_" + fieldId + "']");
    			nowEle.die('change.defaultValue').live("change.defaultValue", function(){
   					eval("liveChange_" + plugId + "(this);");
   				});
    		}
    		if(plugId != undefined && plugId > 0) {
    			var chooseType = nowEle.attr('choosetype');
    			if (chooseType!=2) {
    				$("input[name='au_" + tableId + "_" + pageId + "_" + fieldId + "']").unbind('change.defaultValue').die('change.defaultValue').live("change.defaultValue", function(){
    					eval("liveChange_" + plugId + "(this);");
    				});
    			} else {
    				$("input[name='au_" + tableId + "_" + pageId + "_" + fieldId + "']").omCombo('onValueChange',function(target,newValue,oldValue,event){
    					eval("liveChange_" + plugId + "(this);");
    				});
    			}
    		}
    		...
    		if(plugId != undefined && plugId > 0) {
    			var chooseType = nowEle.attr('choosetype');
    			if (chooseType!=2) {
    				$("input[trueField='" + fieldId + "']").unbind('change.defaultValue').die('change.defaultValue').live("change.defaultValue", function(){
    					eval("liveChange_" + plugId + "(this);");
    				});
    			} else {
    				$("input[trueField='" + fieldId + "']").omCombo('onValueChange',function(target,newValue,oldValue,event){
    					eval("liveChange_" + plugId + "(this);");
    				});
    			}
    		}
    		...
    		if(nowEle.length > 0) {
    			...
    			if(parInput.length > 0) {
	    			...
	    			var eleArr = $("input[name='" + parEleTableId + "_" + fieldId + "']");
	    			if(plugId != undefined && plugId > 0) {
	    				var chooseType = eleArr.attr('choosetype');
    					if (chooseType!=2) {
		    				$("input[name='" + parEleTableId + "_" + fieldId + "']").unbind('change.defaultValue').die('change.defaultValue').live("change.defaultValue", function(){
		    					eval("liveChange_" + plugId + "(this);");
		    				});
		    			} else {
    						$("input[name='" + parEleTableId + "_" + fieldId + "']").omCombo('onValueChange',function(target,newValue,oldValue,event){
    							eval("liveChange_" + plugId + "(this);");
    						});
    					}
    				}
...
function popupCenter(self){//ztx添加弹出层居中定位及窗口高度设置
	var options, a;
	if(self){
		options = self.options;a = self.uiDialog;
		$('.om-dialog.om-widget.om-widget-content.om-corner-all.om-draggable.om-resizable .om-dialog-content.om-widget-content').omScrollbar({thick:8, rightDistance:8, bottomDistance:8});
	}else{
		options = {};options.height='';
		a = $('.om-dialog.om-widget.om-widget-content.om-corner-all.om-draggable.om-resizable');
	};
	var thisTop = a.children('.om-dialog-titlebar.om-widget-header.om-corner-all.om-helper-clearfix'),
	thisCon = a.children('.om-dialog-content.om-widget-content'),
	thisConDiv = thisCon.find('form').parent(),
	thisBottom = a.children('.om-dialog-buttonpane.om-helper-clearfix'),
	thisBodyHeight = parseInt($('body').height()),
	thisBodyWidth = parseInt($('body').width()),
	thisConMarginW = parseInt(thisCon.css('marginLeft')) + parseInt(thisCon.css('marginRight')),
	thisConMarginH = parseInt(thisCon.css('marginTop')) + parseInt(thisCon.css('marginBottom')),
	thisConHeightDIY = 0,
	optionsOf = thisCon.children('.cus_box').length >= 0.1 ? true : false,
	xzcOf = RegExp('选择层').test(thisTop.html()) ? true : false;
	thisCon.siblings().each(function(i,a){if(!RegExp('om-resizable-handle').test(a.className)){thisConHeightDIY += $(a).outerHeight();};});
	var thisBoxWidth = parseInt(thisBodyWidth * .9),
	thisBottomHeight = parseInt(thisBottom.outerHeight()),
	thisBottomDiyH = optionsOf ? thisBottomHeight : 0;
	if(xzcOf){options.height = 'auto';};
	if(RegExp('auto').test(options.height)){a.css({'width':'90%','left':(thisBodyWidth-thisBoxWidth)/2});}else{a.css({'height':options.height});};
	var thisBoxHeight = parseInt(a.outerHeight());
	if(eval(thisConDiv.outerHeight()) < eval(140)){thisBoxHeight = 230}else if(thisBoxHeight >= thisBodyHeight*0.9){thisBoxHeight = thisBodyHeight*0.9};
	var thisBoxTop = (thisBodyHeight-thisBoxHeight) * 0.33;
	if(thisBoxTop<=10){thisBoxTop = 10};
	if(xzcOf){thisBoxTop = 90;};
	a.css({'top':!optionsOf ? thisBoxTop : 0});
	thisBoxTop = optionsOf ? 0 : parseInt(a.css('top'));
	a.css({'max-height':thisBodyHeight - thisBoxTop || 0});
	var thisConMinHeight = parseInt(a.css('height')) - thisConHeightDIY - thisConMarginH + thisBottomDiyH;
	var thisConMaxHeight = thisBodyHeight - thisConHeightDIY - thisConMarginH + thisBottomDiyH - thisBoxTop;
	thisCon.css({'max-height':thisConMaxHeight,'min-height':thisConMinHeight});
	if(RegExp('%').test(options.height)){thisCon.css({'height':thisConMinHeight});};
};
function pdivOfZtreeHeight(){
	var west_panel = $('#west-panel.om-panel-body.om-widget-content'),
	west_panelPrev = west_panel.prev().outerHeight();
	west_panel.height(bodyHeight - west_panelPrev).css('overflow','hidden');
	var widgetContent = $('.om-grid.om-widget.om-widget-content'),
	widgetConPrev = widgetContent.prev(),
	bodyHeight = $('body').height(),
	widgetConH = bodyHeight-widgetConPrev.outerHeight()-2-widgetConPrev.prev().outerHeight();
	widgetConH = widgetContent.closest('#west-panel.om-panel-body.om-widget-content').length >= 0.1 ? widgetConH - west_panelPrev : widgetConH;
	widgetContent.height(widgetConH);
	widgetContent.children('.bDiv').height(widgetConH-widgetContent.children('.hDiv').outerHeight()-widgetContent.children('.pDiv').outerHeight());
	var leftWidgetConPar = $('.om-widget.om-panel.om-resizable.om-borderlayout-region.om-borderlayout-region-west'),
	leftWidgetConPrev = $('.om-panel-header.om-borderlayout-region-header').outerHeight(),
	leftWidgetCon = leftWidgetConPar.children('.om-panel-body.om-widget-content'),
	leftWidgetConTreeOper = leftWidgetCon.children('.edus_treeOper').outerHeight(),
	leftWidgetConZtree = leftWidgetCon.children('.ztree');
	bodyHeight -= 2;
	leftWidgetConPar.height(bodyHeight);
	leftWidgetCon.height(bodyHeight - leftWidgetConPrev);
	leftWidgetConZtree.height(bodyHeight - leftWidgetConPrev - leftWidgetConTreeOper - 5).css({'paddingBottom': '0px','paddingTop': '5px'});
};
function widgetConHei(){
	var searchNone = document.createElement('div'),
	body = document.getElementsByTagName('body')[0],
	searchJ = '', searchQ = '';
	searchNone.className = 'searchNone';
	var gridOf = $('.om-grid.om-widget.om-widget-content').length >= 0.1 ? true : false;
	if(gridOf){searchQ = 'padding: 10px; box-sizing: border-box; '};
	if(gridOf || $('.cus_box').length >= 0.1){searchJ = 'body{' + searchQ + 'overflow: hidden; position:fixed; top:0; left:0; bottom:0; right:0;}'};
	searchNone.innerHTML = '. <style>.om-widget.om-panel .edus_search.om-panel-body.om-widget-content,.searchNone{display:none;}' + searchJ + '</style>';
	if($('body .searchNone').length <= 0.1){body.appendChild(searchNone);};
	pdivOfZtreeHeight();
	var ScroStyle = document.createElement('div'),
	body = document.getElementsByTagName('body')[0];
	ScroStyle.className = 'ScroStyle';
	ScroStyle.innerHTML = '. <style>' + 
	'div.om-scrollbar{z-index: 999999; background-color:#A3A3A3; border-radius: 15px;}.om-grid div.hDivBox,.tbdivbox .rtdiv{overflow: hidden;}' + 
	'div.scrollbar-state-hover{background-color:#4EBFE5; opacity: 1; filter:Alpha(Opacity=100); }.ScroStyle{display:none; width:0px; height:0px;}' + 
	'div.om-scrollbar.scroll_Height:after,div.om-scrollbar.scroll_Width:after{content: " "; display: block; width: 100%; height: 100%; margin-left: -6px;}div.om-scrollbar.scroll_Width:after{margin: -6px 0 0 0;}' + 
	'</style>';
	if($('body .ScroStyle').length <= 0.1){body.appendChild(ScroStyle);};
	setTimeout(function(){
	$("div.bDiv").omScrollbar({thick:10});
	$(".om-widget.om-panel.om-resizable.om-borderlayout-region.om-borderlayout-region-west .ztree").omScrollbar({thick:10});
	$("#tab0.om-panel-body.om-widget-content.om-panel-noheader.om-state-nobd").omScrollbar({thick:10});
	},400);
};
$(window).bind('resize',function(){popupCenter();pdivOfZtreeHeight();});
$(function(){var widgetTint = null, widgetTintIf = 0; clearInterval(widgetTint); widgetTint = setInterval(function(){widgetTintIf++; if(widgetTintIf < 3){widgetConHei();}else{ clearInterval(widgetTint);};},500);});
