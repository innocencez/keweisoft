...
<div class="table-list">
    <table width="100%" cellspacing="0">
        <thead>
			...
			<th width='250' align="center">状态</th>
...
?>   
	...
	<td align="center"><?php if(!empty($d['remarks'])) { ?><font color="green">已备注</font><?php }else{ ?><font color="red">未备注</font><?php } ?></td>
	<td align="center"><?php echo $d['ip']?></td>
	<td align="center"><?php echo date('Y-m-d', $d['datetime'])?></td>
	<td align="center"><a href="javascript:check('<?php echo $formid?>', '<?php echo $d['dataid']?>', '<?php echo safe_replace($d['username'])?>');void(0);"><?php echo L('check')?></a> | <a href="?m=formguide&c=formguide_info&a=public_delete&formid=<?php echo $formid?>&did=<?php echo $d['dataid']?>" onClick="return confirm('<?php echo L('confirm', array('message' => L('delete')))?>')"><?php echo L('del')?></a> | <a href="javascript:remark('<?php echo $formid?>', '<?php echo $d['dataid']?>', '<?php echo safe_replace($d['username'])?>');void(0);">备注</a></td>
	</tr>
<?php 
	}
}
...
function check(id, did, title) {
	...
	window.top.art.dialog({title:'<?php echo L('check')?>--'+title+'<?php echo L('submit_info')?>', id:'edit', iframe:'?m=formguide&c=formguide_info&a=public_view&formid='+id+'&did='+did ,width:'700px',height:'235px'}, function(){window.top.art.dialog({id:'check'}).close()});
}
function remark(id, did, title) {
	window.top.art.dialog({id:'remark'}).close();
	window.top.art.dialog({title:'备注--'+title+'<?php echo L('submit_info')?>', id:'edit', iframe:'?m=formguide&c=formguide_info&a=public_remark&formid='+id+'&did='+did ,width:'700px',height:'400px'}, function(){window.top.art.dialog({id:'remark'}).close()});
}
</script>