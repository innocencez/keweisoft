...
class formguide_info extends admin {
	...
	public function public_remark() {
		...
		//$forminfos_data = $formguide_form->get($info);	
		if(isset($_POST['dosubmit'])){					
			if(!isset($_GET['did']) ||empty($_GET['did'])) showmessage(L('illegal_operation'),HTTP_REFERER);
			$did = intval($_GET['did']);
			$formid = intval($_GET['formid']);
			$info = $_POST['info'];
			$this->db->update($info,array('dataid'=>$did));
			showmessage('remark_success',HTTP_REFERER);			
		}
		if($this->get_siteid() == 1){
			include $this->admin_tpl('formguide_info_remark1');
		}else if($this->get_siteid() == 3){
			include $this->admin_tpl('formguide_info_remark2');
		}else if($this->get_siteid() == 2){
			include $this->admin_tpl('formguide_info_remark3');
		}
		else{
			include $this->admin_tpl('formguide_info_remark');			
		}
