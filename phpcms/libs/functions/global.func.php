...
function wz_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			$multipage .= ' <a href="'.pageurl($urlrule, $curr_page-1, $array).'" id="a1"><img src="'.IMG_PATH.'kwcrm/left.png" class="pageLeft" /></a>';
			$multipage .= '<div class="linkNum">';
			if($curr_page==1) {
				$multipage .= '<div class="linkNumCon"> <span class="linkNumItem1">1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= '<div class="linkNumCon"> <a href="'.pageurl($urlrule, 1, $array).'" class="linkNumItem">1</a>..';
			} else {
				$multipage .= '<div class="linkNumCon"> <a href="'.pageurl($urlrule, 1, $array).'" class="linkNumItem">1</a>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <a href="'.pageurl($urlrule, $i, $array).'" class="linkNumItem">'.$i.'</a>';
			} else {
				$multipage .= ' <span class="linkNumItem1">'.$i.'</span>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<a href="'.pageurl($urlrule, $pages, $array).'" class="linkNumItem">'.$pages.'</a></div></div> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" id="a2" ><img src="'.IMG_PATH.'kwcrm/right.png" class="pageLeft" /></a>';
			} else {
				$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'" class="linkNumItem">'.$pages.'</a></div></div><a href="'.pageurl($urlrule, $curr_page+1, $array).'" id="a2"><img src="'.IMG_PATH.'kwcrm/right.png" class="pageLeft" /></a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span class="linkNumItem1">'.$pages.'</span></div></div> <a href="'.pageurl($urlrule, $curr_page, $array).'" id="a2"><img src="'.IMG_PATH.'kwcrm/right.png" class="pageLeft" /></a>';
		} else {
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'" class="linkNumItem">'.$pages.'</a></div></div> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" id="a2"><img src="'.IMG_PATH.'kwcrm/right.png" class="pageLeft" /></a>';
		}
		$multipage .= '<span class="linkNumAll">共'.$pages.'页</span>';
		$multipage .= '<form name="form2" method="get">';
		$multipage .= '<input type="text" class="linkText" id="linkText" name="page" value="跳转到" />';
		$multipage .= '<input type="hidden" name="m" value="content" />';
		$multipage .= '<input type="hidden" name="c" value="index" />';
		$multipage .= '<input type="hidden" name="a" value="lists" />';
		$multipage .= '<input type="hidden" name="catid" value="11" />';
		$multipage .= '<a href="javascript:document.form2.submit();"><img src="'.IMG_PATH.'kwcrm/go.png" class="pageGo" /></a>';
		$multipage .= '</form> ';
	}
	return $multipage;
}
 ...
 * @return 分页
 */
function index_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
 ...
 * @return 分页
 */
function m_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			$multipage .= ' <a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="np-prev"></a>';
			if($curr_page==1) {
				$multipage .= ' <span>1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <a href="'.pageurl($urlrule, 1, $array).'">1</a>..';
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="np-next"></a>';
			} else {
				$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="np-next"></a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span>'.$pages.'</span> <a href="'.pageurl($urlrule, $curr_page, $array).'" class="np-next"></a>';
		} else {
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="np-next"></a>';
		}
	}
